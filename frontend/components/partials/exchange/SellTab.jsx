...
const SellTab = ({
  ...
  useEffect(() => {
    if (sellData.payAmount && parseFloat(sellData.payAmount) > 0) {
      const calc = calculateSellAmount(parseFloat(sellData.payAmount));
      setReceiveAmount(formatNumber(calc.receiveAmount, 2));
      setCalculation(calc);
    } else {
      setReceiveAmount('');
      setCalculation(null);
    }
  }, [sellData.payAmount, sellData.payCurrency, sellData.receiveCurrency, calculateSellAmount, formatNumber]);
  ...
  const handleSellAmountChange = (e) => {
    ...
    setSellData(prev => ({ ...prev, payAmount: value }));
  };

  const getSelectedPayCurrency = () => {
    return cryptoCurrencies.find(c => c.code === sellData.payCurrency) || cryptoCurrencies[0];
  };

  const getSelectedReceiveCurrency = () => {
    return fiatCurrencies.find(c => c.code === sellData.receiveCurrency) || fiatCurrencies[0];
  };


  const getAvailableCryptos = () => {
    return cryptoCurrencies.filter(c => c.balance > 0);
  };
  ...
  return (
    <form onSubmit={onSubmit} className="relative">
      {/* Header */}
      <div className="text-center text-red-600 dark:text-red-400 font-semibold border-b border-red-600 dark:border-red-400 py-3 mb-0">
        VENDER
      </div>
      ...
      <div className="p-4 pb-2">
        <div className="border border-gray-300 dark:border-slate-600 rounded px-4 py-0">
          ...
          <div className="flex justify-between items-center relative">
            ...
            <div className="flex items-center">
              ...
              <button
                ...
                onClick={() => onDropdownToggle('sell_pay')}
              >
                <img 
                  ...
                  className="mr-2"
                />
                <span className="mr-1">{getSelectedPayCurrency().code}</span>
                <ChevronDown size={16} />
              </button>
            </div>

            {/* Sell Currency Dropdown */}
            {openDropdown === 'sell_pay' && (
              <div className="absolute top-full right-0 mt-1 bg-white dark:bg-slate-800 shadow-lg rounded-md border border-gray-200 dark:border-slate-600 z-20 min-w-[250px]">
                <ul className="py-1">
                  {getAvailableCryptos().map(currency => (
                    <li
                      key={currency.code}
                      className="px-3 py-2 hover:bg-gray-100 dark:hover:bg-slate-700 cursor-pointer flex items-center justify-between"
                      onClick={() => onCurrencySelect('sell', 'payCurrency', currency)}
                    >
                      <div className="flex items-center">
                        {currency.icon === 'pix' ? (
                          <div className="mr-3">
                      ...
                      <div className="text-xs text-gray-500 dark:text-gray-400">
                        {formatNumber(currency.balance, 6)} {currency.code}
                      </div>
                    </li>
                  ))}
                </ul>
          ...
          <div className="flex text-xs text-gray-500 dark:text-gray-400 mt-1 mb-3">
            ...
            <span>{formatNumber(getAvailableBalance(sellData.payCurrency), 6)}</span>
            <span className="ml-1">{sellData.payCurrency}</span>
          </div>
        </div>
      </div>
        ...
        <div className="border border-gray-300 dark:border-slate-600 rounded px-4 py-0">
              ...
              <Tooltip
                content={
                  <div className="text-sm">
                    <div className="font-bold text-center mb-2">Informações da Venda</div>
                    <hr className="my-2 border-gray-600" />
                    <div className="space-y-1">
                      <div className="flex justify-between">
                        <span>Preço:</span>
                        <span>1 {sellData.payCurrency} ≈ {formatNumber(calculation.price, 2)} {sellData.receiveCurrency}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Vender:</span>
                        <span>≈ {formatNumber(calculation.sellAmount)} {sellData.payCurrency}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Taxa:</span>
                        <span>≈ {formatNumber(calculation.fee, 2)} {sellData.receiveCurrency}</span>
                      </div>
                      <div className="flex justify-between font-semibold">
                        <span>Receber:</span>
                        <span>≈ {formatNumber(calculation.receiveAmount, 2)} {sellData.receiveCurrency}</span>
                      </div>
                    </div>
                    <hr className="my-2 border-gray-600" />
          ...
          <div className="flex justify-between items-center relative">
            ...
            <button
              type="button"
              className="flex items-center px-0 py-2 bg-transparent border-none text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white"
              onClick={() => onDropdownToggle('sell_receive')}
            >
                ...
                <ul className="py-1">
                  {fiatCurrencies.map(currency => (
                    <li
                      ...
                      onClick={() => onCurrencySelect('sell', 'receiveCurrency', currency)}
                    >
                      {currency.icon === 'pix' ? (
                        <div className="mr-3">
                          <PIXIcon />
      ...
      <div className="px-4 pb-4">
        <Button
          ...
          isLoading={loading}
          disabled={!sellData.payAmount || parseFloat(sellData.payAmount) <= 0}
        >
          Vender {getSelectedPayCurrency().code}
        </Button>
      </div>
