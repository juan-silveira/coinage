...
const FirstAccessPageContent = () => {
  ...
  // Usar dados do usuário diretamente do store
  ...
  useEffect(() => {
    if (!userId) {
      ...
      // Usar o alias da empresa do usuário logado para o redirect
      const userCompanyAlias = user?.company?.alias || 'coinage';
      router.push(`/login/${userCompanyAlias}`);
    }
  }, [userId, user, router, showError]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!userId) {
        showError('Erro: Dados do usuário não encontrados. Faça login novamente.');
        return;
      }
      
      // Garantir formato ISO (YYYY-MM-DD) para o backend
      const isoBirthDate = (() => {
        try {
          ...
          // Fallback para entradas dd/MM/yyyy
          const match = formData.birthDate.match(/^(\d{2})\/(\d{2})\/(\d{4})$/);
          if (match) {
      ...
      const result = await authService.completeFirstAccess(requestData);
      ...
      // Mostrar mensagem mais específica do backend se disponível
  ...
  const handleLogout = () => {
    ...
    // Usar o alias da empresa do usuário para redirect
    router.push(`/login/${userCompanyAlias}`);
  };
            ...
            <div className="space-y-6">
              ...
              <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 p-4 rounded-lg">
                ...
                <div className="text-sm text-yellow-700 dark:text-yellow-300 space-y-2">
                  ...
                  <p>• Transações em blockchain são irreversíveis</p>
                  <p>• Nunca compartilhe sua senha com terceiros</p>
                  <p>• Ative o 2FA para maior segurança</p>
                </div>
              </div>
            </div>
